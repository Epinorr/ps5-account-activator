...
/*
 * PS5 sign-in state check.
 *
 * The PS5 path in this payload writes REG_KEY_SIGNIN_FLAG from config.dat's
 * 0x1F8 value. The supplied generator defines that value as 0x02.
 *
 * To avoid skipping a real sign-in because of an unrelated registry value,
 * require:
 *   1) a non-zero account ID,
 *   2) sign-in flag == 0x02,
 *   3) the registry online ID is populated.
 */
#define NP_SIGNED_IN_STATE 2

static int is_ps5_fake_signed_in(int account_numb, const char *username,
                                 uint64_t account_id) {
    if (account_numb < 1 || account_numb > 16 || account_id == 0)
        return 0;

    uint32_t off = (account_numb - 1) * 65536;
    int32_t signin_state = 0;
    char online_id[32] = {0};

    if (sceRegMgrGetInt(REG_KEY_SIGNIN_FLAG + off, &signin_state) != 0)
        return 0;

    if (signin_state != NP_SIGNED_IN_STATE)
        return 0;

    if (sceRegMgrGetStr(REG_KEY_ONLINE_ID + off, online_id, sizeof(online_id)) != 0)
        return 0;

    online_id[sizeof(online_id) - 1] = '\0';

    if (username == NULL || username[0] == '\0')
        return online_id[0] != '\0';

    return strcmp(online_id, username) == 0;
}

static void notify_epinor(const char *text) {
    char msg[3075] = {0};
    snprintf(msg, sizeof(msg), "NP Fake Signin\n\n%s\n\nCoded by EPINOR",
             text != NULL ? text : "");
    notify("%s", msg);
}
...
int main() {
    printf("EPINOR NP Fake Signin\n");
    printf("Account activation + offline sign-in\n\n");

    OrbisUserServiceInitializeParams params = { .priority = 0x2BC };
    int32_t init_ret = sceUserServiceInitialize(&params);
    if (init_ret != 0) {
        notify_epinor("User Service initialization failed");
        return 1;
    }
    ...
    /*
     * Step 1: Ensure the foreground account is activated.
     * account_activator_run() is a no-op when Account ID is already non-zero.
     */
    int activation_changed = 0;
    int activation_error = 0;
    int activated_slot = -1;
    uint64_t activated_account_id = 0;

    int activation_ret = account_activator_run(
        userName, &activation_changed, &activation_error,
        &activated_slot, &activated_account_id);

    if (activation_ret != ACCOUNT_ACTIVATOR_OK) {
        printf("Account activation failed: 0x%x\n",
               (uint32_t)(activation_error != 0 ? activation_error : activation_ret));
        notify_epinor("Account activation failed");
        sceUserServiceTerminate();
        return 1;
    }

    if (activated_slot < 1 || activated_account_id == 0) {
        printf("Account activation did not return a valid account state\n");
        notify_epinor("Invalid account activation state");
        sceUserServiceTerminate();
        return 1;
    }

    if (activation_changed)
        printf("Account activated (slot %d, id=0x%lx)\n",
               activated_slot, activated_account_id);
    else
        printf("Account already activated (slot %d, id=0x%lx)\n",
               activated_slot, activated_account_id);
    ...
    /* Step 2: Do not re-run Fake Signin if the current user is already signed in. */
#ifdef PS5
    if (is_ps5_fake_signed_in(activated_slot, userName, activated_account_id)) {
        printf("User is already signed in. Skipping NP Fake Signin.\\n");
        notify_epinor("Already signed in");
        sceUserServiceTerminate();
        return 0;
    }
#endif

    /* Reuse the account state returned by the activator. */
    int account_numb = activated_slot;
    uint64_t acct_id64 = activated_account_id;
    printf("Proceeding with NP Fake Signin (slot %d, id: 0x%lx)\\n",
    ...
    notify_epinor("Signed in successfully\nReboot recommended to apply changes.");
